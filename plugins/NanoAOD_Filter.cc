...
bool NanoAOD_Filter::filter( edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto &ele : *eles)
	...
	if(ele.pt()>60 and ele.electronID("mvaEleID-Fall17-iso-V1-wp80"))
	{
	//std::cout<<"FAIL ele iso pt "<<ele.pt()<<std::endl;
	return 0;
	}
	if(ele.pt()>150 and ele.electronID("mvaEleID-Fall17-noIso-V1-wp80"))
	{
