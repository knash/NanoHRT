...
class HOTVRProducer : public edm::stream::EDProducer<> {
  ...
  const std::string recoSubjetCollName_;
...
HOTVRProducer::HOTVRProducer(const edm::ParameterSet& iConfig):
  ...
  recoSubjetCollName_("RecoSubJets"),
  doRekey_(iConfig.exists("doRekey") ? iConfig.getParameter<bool>("doRekey") : false),
  unGroom_(iConfig.exists("unGroom") ? iConfig.getParameter<bool>("unGroom") : false)
{
  ...
  produces<reco::PFJetCollection>(recoSubjetCollName_);
...
void HOTVRProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  auto recoSubjetCollection = std::make_unique<reco::PFJetCollection>();
  ...
  for (unsigned int i = 0; i < hotvr_jets.size(); ++i) {
    ...
    for (uint s=0; s<subjets.size(); s++) {
      ...
      reco::PFJet recoSubjet;
      recoSubjet.setP4(subjet.p4());
      recoSubjet.setJetArea(subjet.jetArea());
      recoSubjetCollection->push_back(recoSubjet);
    }
  ...
  iEvent.put(std::move(recoSubjetCollection), recoSubjetCollName_);
